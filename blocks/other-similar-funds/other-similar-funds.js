...
export default function decorate(block) {
  ...
  const FundSimilar = dataCfObj.filter((el, infex) => {
                ...
                onchange: (event) => {
                  FundSimilar.forEach((elem) => {
                    if (event.target.getAttribute('schemecode') === elem.schcode) {
                      const cgarValue = elem.returns[0][event.target.value];
  ...
  const funcontainrClassList = [...block.classList];
